DROP TABLE IF EXISTS {proccess_zone}.pmod_base_cruce_0 PURGE;
CREATE TABLE {proccess_zone}.pmod_base_cruce_0
...
LEFT JOIN {proccess_zone}.pmod_maestro_cuotas_obligaciones t3
      ...
      AND t1.f_cruce = CAST(t3.fecha_corte/100 AS INT)
...
COMPUTE STATS {proccess_zone}.pmod_base_cruce_0;
...
SELECT DISTINCT
    ...
    t2.prob_propension AS pd_prob_propension,
    t2.prob_alrt_temprana AS pd_prob_alrt_temprana,
    t2.prob_auto_cura AS pd_prob_auto_cura

FROM {proccess_zone}.pmod_base_cruce_0 t1 
LEFT JOIN {proccess_zone}.pmod_pd_obligaciones t2
       ON t1.nit_enmascarado = t2.nit_enmascarado
      AND t1.num_oblig_enmascarado = t2.num_oblig_enmascarado
      AND t1.f_cruce = t2.fecha_corte
;
